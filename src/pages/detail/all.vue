...
<div>
    <div id="box" class="clearfix">
        <div id="sale_wrapper" v-for="item in alldata" :key="item">
            <div>
                <header>
                    <img v-lazy="insertStr(item.foods[0].image_path)">
...
<script>
import jquery from "./jquery-2.2.3.js"
import vue from 'vue'
export default {
    name: 'name',
    data: function() {
    ...
    created() {
        vue.http.get('../../../static/data/sale.json').then(data => {
            this.alldata = data.body.query_list
        ...
        window.onscroll = () => {
            ...
            var top = Number($("#box")[0].offsetHeight);
            // console.log(top)
            console.log(winH + scrollTop - top)
            if (winH + scrollTop - top >= 0) {
                vue.http.get('../../../static/data/sale.json').then(data => {
                    console.log(self.alldata)
                    for (var i = 0; i < data.length; i++) {
                        self.alldata.push(data.body.query_list[i])
                    }
                    console.log(this.alldata)
                }, err => {
                    console.log(err)
